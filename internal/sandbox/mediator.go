...
type Approver interface {
	Approve(context.Context, policy.ToolCall, policy.PolicyDecision) (bool, error)
}
...
func (m *Mediator) Execute(ctx context.Context, call policy.ToolCall, handler Handler) (string, error) {
	...
	if err != nil {
		...
		return denied(policy.PolicyDecision{
...
func denied(decision policy.PolicyDecision) string {
	return fmt.Sprintf("Error: execution denied. Reason: %s", decision.Reason)
}

func (m *Mediator) validateCall(call policy.ToolCall) (policy.ToolCall, error) {
