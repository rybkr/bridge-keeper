...
var RuntimeGeminiAgent *GeminiAgent = nil

func NewGeminiAgent() *GeminiAgent {
	client, err := genai.NewClient(env.ModelContext, &genai.ClientConfig{
	...
	return &GeminiAgent{
		...
		lastPath:     env.RuntimeRegistry.WorkspaceRoot,
		mediator:     env.EMediator,
		registry:     env.RuntimeRegistry,
	}
}
...
func (agent *GeminiAgent) SendGeminiMessage(prompt string) (string, error) {
	if agent.chatSession == nil || agent.isConcise != env.Concise {
		config := agent.getChatConfig()
		chat, err := agent.client.Chats.Create(env.ModelContext, agent.currentModel, config, nil)
		if err != nil {
			return "", fmt.Errorf("failed to create chat session: %w", err)
		}
		agent.chatSession = chat
		agent.isConcise = env.Concise
	}

	resp, err := agent.chatSession.SendMessage(env.ModelContext, genai.Part{Text: prompt})
	if err != nil {
		return "", err
	}
	...
	for {
		...
		for _, part := range resp.Candidates[0].Content.Parts {
			if funcCall := part.FunctionCall; funcCall != nil {
				...
				responseContent, err := agent.mediator.Execute(env.ModelContext, toolCall, func(_ context.Context, args map[string]any) (string, error) {
					return agent.executeTool(env.ModelContext, funcCall.Name, args)
				})
				if err != nil {
					return "", err
				}
		...
		resp, err = agent.chatSession.SendMessage(env.ModelContext, functionResponses...)
		if err != nil {
			return "", err
		}
	}
...
func (agent *GeminiAgent) getChatConfig() *genai.GenerateContentConfig {
